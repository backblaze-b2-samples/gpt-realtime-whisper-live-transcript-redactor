...
import { useSessionActivity } from "@/lib/queries";
...
export function SessionChart() {
  const { data: activity, error, refetch } = useSessionActivity(7);
  ...
  return (
    <Card>
      <CardHeader className="border-b border-border py-4 px-5">
        <CardTitle className="card-title">Session Activity</CardTitle>
      ...
      <CardContent className="p-5">
        {error ? (
          ...
          <EmptyState
            ...
            description="Start a recording to see activity trends here."
          />
        ) : (
            ...
            >
              <defs>
                <linearGradient id="sessions-fill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="var(--color-sessions)" stopOpacity={0.95} />
                  <stop offset="100%" stopColor="var(--color-sessions)" stopOpacity={0.55} />
                </linearGradient>
              </defs>
              <CartesianGrid
                vertical={false}
