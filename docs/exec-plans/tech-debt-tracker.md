...
| Remove legacy B2 env fallbacks | `B2_KEY_ID`, `B2_ENDPOINT`, and `B2_PUBLIC_URL` remain tolerated for one release cycle so rolling deploys and local `.env` files can migrate safely. | After downstream deployments have moved to `B2_APPLICATION_KEY_ID`, derived endpoints from `B2_REGION`, and `B2_PUBLIC_URL_BASE`, remove the legacy fallbacks from `Settings`, `scripts/doctor.mjs`, and docs. | Medium | Open |
