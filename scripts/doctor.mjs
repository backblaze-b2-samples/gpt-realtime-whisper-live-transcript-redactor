...
const AUDIO_WORKLET_PATH = resolve(
  REPO_ROOT,
...
function checkEnv() {
  ...
  // OPENAI_API_KEY is a soft requirement — the /files explorer, dashboard,
  // and settings still work without it, but /record will refuse to start.
  // Warn rather than fail so contributors can iterate on the rest.
  if (!env.OPENAI_API_KEY || PLACEHOLDERS.has(env.OPENAI_API_KEY)) {
    warn(
      "OPENAI_API_KEY is not set — /record will not connect to OpenAI Realtime",
...
function checkAudioWorklet() {
  if (!existsSync(AUDIO_WORKLET_PATH)) {
    fail(
      "apps/web/public/audio-worklet.js is missing — /record cannot capture PCM16 audio",
      "Restore the file from the starter template; it MUST be served at /audio-worklet.js (not bundled)",
    );
  }
}
