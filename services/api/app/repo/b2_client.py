...
def _public_url(key: str) -> str | None:
    ...
    base_url = settings.b2_public_url_base.strip().rstrip("/")
    if not base_url:
        return None
...
def get_s3_client():
    ...
    if settings.b2_region:
        kwargs["region_name"] = settings.b2_region
...
def get_inline_presigned_url(key: str, expires_in: int = 600) -> str:
    """Return a presigned URL with `inline` disposition (for in-page playback).
